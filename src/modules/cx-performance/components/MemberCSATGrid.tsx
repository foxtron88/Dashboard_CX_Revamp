...
export default function MemberCSATGrid({ records, hideHeader, allRecords, fromMonth, toMonth }: Props) {
  ...
  function renderCard(s: (typeof memberCards)[0]) {
  ...
  function renderHorizontalAllMembersCard(s: (typeof memberCards)[0]) {
    const accent = '#10b981';
    return (
      <div className="glass-card relative overflow-hidden transition-all duration-300 flex flex-col md:flex-row items-center justify-between gap-6 p-6 mb-6" style={{ borderColor: `${accent}40` }}>
        {/* Left: Header */}
        <div className="flex flex-col items-center md:items-start text-center md:text-left min-w-[180px]">
          <p className="text-[24px] font-extrabold text-white leading-tight mb-2 uppercase tracking-wide">All Members</p>
          <p className="text-[11px] font-medium text-[var(--text-muted)] bg-[var(--bg-secondary)] px-3 py-1 rounded-full border border-[var(--glass-border)] shadow-sm inline-block">
            {s.total.toLocaleString()} responses
          </p>
        </div>

        {/* Middle: Big Score */}
        <div className="flex flex-col items-center justify-center text-center px-8 md:border-l md:border-r border-[var(--glass-border)] min-w-[220px]">
          <div className="relative w-full flex flex-col items-center">
            {s.csatScore > 0 && s.csatScore < target && (
              <div className="absolute -top-4 -right-4 bg-red-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-full shadow-sm flex items-center gap-1 animate-pulse" title={`Below Target (${target.toFixed(2)})`}>
                <span>⚠️</span> Below Target
              </div>
            )}
            <p className="text-[10px] text-[var(--text-muted)] uppercase tracking-widest font-semibold mb-1">CSAT Score</p>
            <p className="text-5xl font-extrabold mb-1 flex items-baseline justify-center gap-1" style={{ color: getScoreColor(s.csatScore), fontFamily: 'var(--font-display)' }}>
              <span>{s.csatScore > 0 ? s.csatScore.toFixed(2) : '—'}</span>
              {s.csatScore > 0 && <span className="text-lg font-bold text-[var(--text-muted)]">/5</span>}
            </p>
            <p className="text-[11px] font-medium text-[var(--text-muted)]">Target: {target.toFixed(2)}</p>
          </div>
          {s.momDiff !== null && (
            <div className="flex items-center justify-center mt-3">
              <div className={`flex items-center gap-0.5 text-[11px] font-bold ${s.momDiff >= 0 ? 'text-[var(--accent-success)]' : 'text-[var(--accent-danger)]'}`}>
                {s.momDiff >= 0 ? '▲' : '▼'} {Math.abs(s.momDiff).toFixed(2)} <span className="text-[var(--text-muted)] font-medium">MoM</span>
              </div>
            </div>
          )}
        ...
        <div className="flex-1 w-full grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={d.label} className="flex flex-col items-center justify-center bg-[var(--bg-tertiary)] rounded-xl py-3 px-2 border border-[var(--glass-border)] transition-all hover:bg-[var(--glass-bg)] hover:-translate-y-1">
              <span className="text-[10px] font-extrabold uppercase tracking-widest mb-1" style={{ color: d.color }}>
                {d.label}
              </span>
              <div className="flex items-center gap-1">
                ...
                <span className="text-xl font-bold" style={{ color: getScoreColor(d.val), fontFamily: 'var(--font-display)' }}>
                  {d.val > 0 ? d.val.toFixed(2) : '—'}
                </span>
              </div>
            </div>
    ...
    <section className="mt-10 animate-in">
      ...
      {allMembersCard && (
        <div className="w-full">
          {renderHorizontalAllMembersCard(allMembersCard)}
        </div>
      )}

      {/* Per-Member Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        {memberCards.map(s => renderCard(s))}
      </div>
    </section>
  );
}
